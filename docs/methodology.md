...
Window length respects the dynamic-connectivity lower bound: it must exceed the longest wavelength retained in the signal (Leonardi & Van De Ville, 2015). ATHENA band-pass filters at 0.009 Hz, which puts that floor near 111 s, so the **recommended** physical window is 120 s where the scan allows it — pass it explicitly with `--window-seconds 120` (the notebook's example configs already do). Sites with a short scan (OHSU, 185 s) yield fewer windows at a given window/step, which can make dynamic estimates less stable — that is a methodological consideration for choosing `--representation static` explicitly on such sites, not something the pipeline does on its own.

**The code has no automatic fallback to `static` for short scans, for OHSU or any other site.** `--representation` is always whatever is passed on the command line; omitting window/step arguments does not change the representation, it only falls back to the legacy bare-CLI window default (see below). In the paper's reference campaign, OHSU was evaluated windowed, with the same requested 120 s / 12 s as the other three sites, which resolves to 6 windows given its shorter scan — not the static representation. See [`docs/paper_reference_configuration.md`](paper_reference_configuration.md) for the configuration actually used in the manuscript.
